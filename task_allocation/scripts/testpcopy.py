...
if __name__ == '__main__':
    try:
        ...
        if planner.is_ready():
            # --- Test Parameters ---
            # Define a LIST of ROIs in world coordinates (adjust to your map's coords)
            rois_test_list = [
                {'min_x': 0.0, 'min_y': 0.0, 'max_x': 3.0, 'max_y': 3.0},  # ROI 1
                {'min_x': 3.5, 'min_y': 0.0, 'max_x': 6.5, 'max_y': 3.0},  # ROI 2
                {'min_x': 0.0, 'min_y': 3.5, 'max_x': 3.0, 'max_y': 6.5}   # ROI 3
                # Add more ROIs as needed, ensure they are distinct enough or test connection logic elsewhere
            ]
            ...
            rospy.loginfo(f"Starting path planning test for {len(rois_test_list)} ROIs...")

            for i, current_roi in enumerate(rois_test_list):
                rospy.loginfo(f"Planning for ROI {i+1}/{len(rois_test_list)}: {current_roi}")
                
                waypoints_for_roi = planner.plan_boustrophedon_path(
                    roi=current_roi,
                ...
                if waypoints_for_roi:
                    rospy.loginfo(f"  Generated {len(waypoints_for_roi)} waypoints for ROI {i+1}.")
                    # Simple concatenation for the test script.
                    # For more advanced connection logic (e.g., planning paths between ROIs),
                    # refer to scripts like multi_roi_publisher.py.
                    all_generated_waypoints.extend(waypoints_for_roi)
                else:
                    rospy.logwarn(f"  No waypoints generated for ROI {i+1}. Check ROI parameters and C-Space for this region.")

            if all_generated_waypoints:
                rospy.loginfo(f"Total waypoints generated for all ROIs: {len(all_generated_waypoints)}")
                # Publish combined waypoints as a Path for visualization in RViz
                path_pub = rospy.Publisher('/coverage_path', Path, queue_size=1, latch=True)
                path_msg = Path()
                path_msg.header.stamp = rospy.Time.now()
                path_msg.header.frame_id = planner.map_frame_id # Use map_frame_id from planner
                ...
                for i, (wx, wy) in enumerate(all_generated_waypoints):
                    pose_stamped = PoseStamped() # For Path message
                    ...
                    pose_stamped.pose.orientation.w = 1.0 # Neutral orientation for Path
                    path_msg.poses.append(pose_stamped)

                    point_pose = Pose() # For PoseArray message
                    point_pose.position.x = wx
                    point_pose.position.y = wy
                    point_pose.position.z = 0.0
                    
                    # Calculate orientation based on the next point in the combined list
                    if i + 1 < len(all_generated_waypoints):
                        next_wx, next_wy = all_generated_waypoints[i+1]
                        yaw = math.atan2(next_wy - wy, next_wx - wx)
                    elif i > 0: # For the very last point, use orientation from previous segment
                        prev_wx, prev_wy = all_generated_waypoints[i-1]
                        yaw = math.atan2(wy - prev_wy, wx - prev_wx)
                    else: # Single point path (or first point if only one)
                        yaw = 0.0
                    
                    q = quaternion_from_euler(0, 0, yaw)
                    point_pose.orientation.x = q[0]
                    point_pose.orientation.y = q[1]
                    point_pose.orientation.z = q[2]
                    point_pose.orientation.w = q[3]
                    waypoints_msg.poses.append(point_pose)
                
                waypoints_pub.publish(waypoints_msg)
                path_pub.publish(path_msg)
                rospy.loginfo(f"Published {len(path_msg.poses)} combined waypoints to /coverage_path and /coverage_waypoints for visualization.")
                
                # Publish C-Space map (it's calculated based on the last ROI's parameters if called within loop,
                # or based on the first successful _calculate_cspace call if plan_boustrophedon_path is robust)
                # For simplicity, we publish it once after all planning. It will reflect the C-Space
                # used for the *last* ROI for which C-Space was computed, or the first if it was reused.
                if planner._cspace_calculated: # Check if C-Space was actually calculated
                    planner.publish_cspace_map()
                else:
                    rospy.logwarn("C-Space was not calculated during planning for any ROI, not publishing C-Space map.")
            else:
                rospy.logwarn("Path planning returned no waypoints for any ROI. Check parameters and map.")
